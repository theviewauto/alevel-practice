...
function App() {
  const chooseSubject = (subjectName: string) => {
    alert(`คุณเลือกวิชา ${subjectName}\n\nระบบเลือกวิชาและแบบทดสอบวัดระดับจะเพิ่มในขั้นถัดไป`);
  };
          ...
          {subjects.map((subject) => (
            <article className={`subject-card ${subject.color}`} key={subject.id}>
              ...
              <button
                ...
                onClick={() => chooseSubject(subject.title)}
              >
                เริ่มฝึกวิชานี้
                <ArrowRight size={18} />
              </button>
